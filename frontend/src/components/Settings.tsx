      ...
      <div className="settings-section">
            ...
            return (
              <div key={r.id} className="retailer-row">
                <div className="retailer-info">
                  ...
                  {acc?.email ? (
                    <span className="retailer-email">{acc.email} {acc.has_password ? '(configured)' : '(no password)'}{acc.has_password && !acc.has_cvv && (r.id === 'target' || r.id === 'walmart' || r.id === 'bestbuy' || r.id === 'costco' || r.id === 'samsclub') ? ' — missing CVV' : ''}{r.id === 'bestbuy' && acc.has_password && (!acc.has_phone_last4 || !acc.has_account_last_name) ? ' — missing verification info' : ''}</span>
                  ) : (
        ...
        {editRetailer && (
          <div className="retailer-edit">
            ...
            {(editRetailer === 'target' || editRetailer === 'walmart' || editRetailer === 'bestbuy' || editRetailer === 'costco' || editRetailer === 'samsclub') && (
              <div className="setting-field">
