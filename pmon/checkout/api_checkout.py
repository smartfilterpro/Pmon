...
class ApiCheckout:
    ...
    async def _checkout_costco(
        self, url: str, product_name: str, profile: Profile, creds: AccountCredentials
    ) -> CheckoutResult:
        """Costco API checkout flow.

        Uses Costco's web checkout API endpoints:
        - Auth: OAuth via /OAuthLogonCmd (session-based, requires membership)
        - Token: /gettoken for CSRF / session tokens
        - Cart: /AjaxAddOrderItemToShoppingCart or GraphQL
        - Checkout: /OrderProcessCmd

        Costco requires an active membership for most purchases.
        Session cookie import is the most reliable auth method since Costco
        uses Akamai bot protection on login pages.
        """
        client = self._get_client("costco")
        retailer = "costco"
        ...
        # Session cookies required — Costco uses membership-gated checkout
        has_session = bool(self._session_cookies.get("costco"))
        if not has_session:
            logger.warning("Costco: no session cookies — attempting OAuth login")
            login_ok = await self._costco_oauth_login(client, creds)
            if not login_ok:
                return CheckoutResult(
                    url=url, retailer=retailer, product_name=product_name,
                    status=CheckoutStatus.FAILED,
                    error_message=(
                        "Costco login failed (Akamai may be blocking). "
                        "Import session cookies via Dashboard > Accounts > Costco > Import Cookies"
                    ),
                )

        # Step 1: Validate session
        session_valid = await self._costco_validate_session(client)
        if not session_valid:
            return CheckoutResult(
                url=url, retailer=retailer, product_name=product_name,
                status=CheckoutStatus.FAILED,
                error_message="Costco session expired — re-import cookies via Dashboard",
            )

        # Step 2: Get CSRF token
        csrf_token = await self._costco_get_token(client)

        # Step 3: Add to cart
        cart_ok = await self._costco_add_to_cart(client, item_number, csrf_token)
        if not cart_ok:
            return CheckoutResult(
                url=url, retailer=retailer, product_name=product_name,
                status=CheckoutStatus.FAILED,
                error_message="Failed to add item to Costco cart",
            )

        # Step 4: Initiate checkout
        checkout_ok = await self._costco_checkout(client, csrf_token)
        if checkout_ok:
            return CheckoutResult(
                url=url, retailer=retailer, product_name=product_name,
                status=CheckoutStatus.SUCCESS,
                error_message="Checkout initiated — check your Costco account for order confirmation",
            )

        return CheckoutResult(
            url=url, retailer=retailer, product_name=product_name,
            status=CheckoutStatus.FAILED,
            error_message="Costco checkout failed",
        )

    async def _costco_oauth_login(self, client: httpx.AsyncClient, creds: AccountCredentials) -> bool:
        """Attempt Costco OAuth login via /OAuthLogonCmd.

        Costco login flow:
        1. Load login page to get initial cookies + form tokens
        2. POST credentials to /OAuthLogonCmd
        3. Receive session cookies on success

        This often fails due to Akamai bot detection. Cookie import is preferred.
        """
        if not creds.email or not creds.password:
            return False

        try:
            # Step 1: Load login page to warm cookies
            resp = await client.get(
            ...
            if resp.status_code == 403:
                logger.warning("Costco: Akamai blocked login page")
                return False

            # Step 2: Submit credentials via OAuth endpoint
            login_resp = await client.post(
                "https://www.costco.com/OAuthLogonCmd",
                data={
                    "logonId": creds.email,
                    "logonPassword": creds.password,
                    "rememberMe": "true",
                },
                headers={
                    **HEADERS,
                    ...
                    "Referer": "https://www.costco.com/LogonForm",
                },
            )

            # Costco redirects to homepage on success (302) or returns 200 with krypto param
            if login_resp.status_code in (200, 302):
                ...
                if any("login" in name.lower() or "auth" in name.lower() or "session" in name.lower()
                       for name in cookies):
                    logger.info("Costco: OAuth login successful")
                    return True

                # Check if redirect URL contains krypto (success indicator)
                location = login_resp.headers.get("location", "")
                if "krypto=" in location:
                    logger.info("Costco: OAuth login successful (krypto redirect)")
                    return True

            logger.warning("Costco: OAuth login failed (HTTP %d)", login_resp.status_code)
            return False

        except Exception as exc:
            logger.error("Costco OAuth login error: %s", exc)
            return False

    async def _costco_validate_session(self, client: httpx.AsyncClient) -> bool:
        """Validate Costco session by checking account status."""
        try:
            # Try the token endpoint — it returns user info if authenticated
            resp = await client.get(
            ...
            if resp.status_code == 200:
                try:
                    data = resp.json()
                    # Authenticated users get a token response
                    if data.get("loggedIn") or data.get("token") or data.get("userId"):
                        logger.info("Costco: session is valid")
                        return True
                except Exception:
                    pass

            # Fallback: try loading account page
            ...
            if acct_resp.status_code == 200:
                logger.info("Costco: session valid (via myaccount)")
                return True

            return False
        except Exception as exc:
            logger.debug("Costco session validation error: %s", exc)
            return False

    async def _costco_get_token(self, client: httpx.AsyncClient) -> str:
        """Get CSRF/session token from Costco."""
        try:
            resp = await client.get(
                "https://www.costco.com/gettoken",
                headers={**HEADERS, "Accept": "application/json"},
            ...
            logger.debug("Costco token fetch error: %s", exc)
        return ""

    async def _costco_add_to_cart(self, client: httpx.AsyncClient, item_number: str, csrf_token: str) -> bool:
        """Add item to Costco cart."""
        headers = {
            **HEADERS,
            "Content-Type": "application/json",
            "Origin": "https://www.costco.com",
        ...
        # Try Ajax add-to-cart endpoint
        try:
            resp = await client.post(
                "https://www.costco.com/AjaxAddOrderItemToShoppingCart",
                json={
                    "catalogId": "10701",
                    "langId": "-1",
                    "storeId": "10301",
            ...
            if resp.status_code in (200, 201):
                try:
                    data = resp.json()
                    if not data.get("error") and not data.get("errorMessage"):
                        logger.info("Costco: added item %s to cart", item_number)
                        return True
                    logger.warning("Costco: cart add returned error: %s",
                                   data.get("errorMessage", data.get("error", "")))
                except Exception:
                    # Non-JSON 200 may still be success
                    logger.info("Costco: cart add returned 200 (non-JSON)")
                    return True
            elif resp.status_code == 401:
                logger.warning("Costco: session expired during cart add")
                return False
            elif resp.status_code == 403:
                logger.warning("Costco: blocked (403) on cart add — Akamai")
                return False
        except Exception as exc:
            logger.debug("Costco cart add error: %s", exc)

        # Fallback: form-based add to cart
        try:
            resp = await client.post(
                "https://www.costco.com/OrderItemAdd",
                data={
                    "catalogId": "10701",
                    "langId": "-1",
                    "storeId": "10301",
            ...
            if resp.status_code in (200, 201, 302):
                logger.info("Costco: added to cart via form POST")
                return True
        except Exception as exc:
            logger.debug("Costco form cart add error: %s", exc)

        return False

    async def _costco_checkout(self, client: httpx.AsyncClient, csrf_token: str) -> bool:
        """Initiate Costco checkout."""
        headers = {
            **HEADERS,
            "Content-Type": "application/json",
            "Origin": "https://www.costco.com",
            "Referer": "https://www.costco.com/CheckoutCartDisplayView",
        }
        if csrf_token:
            headers["X-CSRF-TOKEN"] = csrf_token

        # Try order process endpoint
        try:
            resp = await client.post(
                "https://www.costco.com/OrderProcessCmd",
                json={"storeId": "10301", "langId": "-1", "catalogId": "10701"},
                headers=headers,
            )
            if resp.status_code in (200, 201, 302):
                logger.info("Costco: checkout initiated via OrderProcessCmd")
                return True
        except Exception as exc:
            logger.debug("Costco OrderProcessCmd failed: %s", exc)
        ...
        try:
            ...
            if resp.status_code == 200 and "checkout" in resp.text.lower():
                logger.info("Costco: checkout page loaded")
                return True
        except Exception:
            pass
