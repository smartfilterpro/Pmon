"""Best Buy checkout flow handler (stub).

CREATED [Mission 3] — Placeholder for Best Buy checkout flow.
...
class BestBuyCheckoutHandler(BaseCheckoutHandler):
    """Best Buy checkout flow handler.

    TODO [Mission 3]: Implement full checkout flow.

    Best Buy checkout flow overview:
    - Add to cart via PDP button
    - Navigate to cart page
    - Click "Checkout" (requires sign-in)
    - Fill/confirm shipping address
    - Fill/confirm payment (saved card + CVV)
    - Review and place order

    Confirmation signals (TODO: validate):
    - URL contains "/checkout/thank-you" or "/site/order-confirmation"
