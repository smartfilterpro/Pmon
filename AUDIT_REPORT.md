...
|-----------|-------|---------|
| `pmon/login/` | `__init__.py`, `base.py`, `target.py`, `walmart.py`, `pokemoncenter.py`, `bestbuy.py`, `costco.py`, `samsclub.py`, `runner.py` | Per-retailer login handlers with `LoginResult`, `LoginStatus`, `BaseLoginHandler` |
| `pmon/queue/` | `__init__.py`, `detector.py`, `handler.py` | Virtual queue detection (Queue-It, custom) and patient wait handler |
| `pmon/checkout_flows/` | `__init__.py`, `base.py`, `target.py`, `walmart.py`, `pokemoncenter.py`, `bestbuy.py`, `costco.py`, `samsclub.py`, `runner.py` | Per-retailer checkout step handlers with `CheckoutStatus.PLACED/UNKNOWN/FAILED` |
| `pmon/` | `rate_limit.py` | Lightweight in-memory rate limiter for auth endpoints |
| `tests/` | `test_login.py`, `test_checkout.py` | Async test suites with mocked Playwright |

**Total new files**: 22 | **Total new lines**: ~4,300

### Files Modified

| File | Change | Mission |
|------|--------|---------|
| `pmon/auth.py:17-24` | JWT secret auto-generates with loud warning if env var missing | Mission 4B |
| `pmon/database.py:164-167,191-201` | CVV wipe migration on startup (PCI-DSS) | Mission 4A |
| `pmon/dashboard/app.py:23,62,70,77,438-444` | Rate limiting on login/register; CVV no longer stored | Mission 4A/4C |
| `pmon/checkout/engine.py:86-88,760,806-816,1616,1982,1998-2010,2051,2237,2567,2590-2602` | Queue detection integration + deprecation comments | Mission 1/2/3 |
| `config/config.example.yaml:1-6` | JWT secret generation instructions + CVV note | Mission 4B |

### Key Architectural Decisions

1. **Login modules don't delete original code** — `_sign_in_target()` etc. remain with `# DEPRECATED` comments. New `pmon/login/runner.py` is the canonical path; migration happens incrementally.

2. **`confirm_order_placed()` never returns FAILED** — After "Place order" click, status is either `PLACED` (confirmation detected) or `UNKNOWN` (timeout, ambiguous). Only `FAILED` if order was definitively not placed (e.g. before clicking).

3. **CVV wipe is automatic** — On next DB startup, any stored CVV values are wiped. Users must re-enter CVV at checkout. The `card_cvv` column remains (SQLite can't drop columns) but is always empty.

4. **Rate limiter is in-memory** — No Redis dependency. Entries auto-prune. Sufficient for single-process deployments (Railway, Docker).

5. **Queue handler never navigates away** — Maintains queue position by only polling/scrolling. Uses human-like idle actions to avoid idle-timeout kicks.

### Test Coverage Added

| Test File | Tests | What's Tested |
|-----------|-------|---------------|
| `test_login.py` | ~15 | LoginResult/LoginStatus, handler dispatch, SESSION_REUSED, verify_authenticated, obstacle detection |
| `test_checkout.py` | ~12 | CheckoutStatus enum values, StepResult creation, confirm_order_placed UNKNOWN behavior, price guard, runner dispatch |
| `test_account_isolation.py` | ~10 | Session path isolation, auth state isolation, context caching (from Phase 1) |

### Remaining TODOs

- [ ] Wire `LoginRunner` into `CheckoutEngine.attempt_checkout()` to replace direct `_sign_in_*` calls
- [ ] Wire `CheckoutFlowRunner` into `CheckoutEngine` to replace direct `_checkout_*` calls
- [ ] Implement Best Buy login (currently stub — reCAPTCHA blocking)
- [ ] Implement Costco and Sam's Club login/checkout (stubs)
- [ ] Add `max_price` field to Product model and database schema
- [ ] Encrypt card_number/card_exp at rest (currently plaintext in SQLite)
- [ ] Add integration test markers and CI configuration

---

## Post-Hardening Regression Risk

| Change | Risk Level | Rollback |
|--------|-----------|----------|
| Login module extraction | **None** — additive, original methods untouched | Delete `pmon/login/` |
| Queue detection in engine.py | **Low** — adds import + if-block after page.goto | Revert 3 blocks in engine.py |
| Checkout flow extraction | **None** — additive, original methods untouched | Delete `pmon/checkout_flows/` |
| CVV wipe migration | **Medium** — users lose stored CVV on next startup | Restore from DB backup; requires re-entry |
| JWT secret change | **Medium** — active sessions invalidated on restart if env var not set | Set `PMON_JWT_SECRET` env var |
| Rate limiting | **None** — additive middleware | Remove import + 2 lines in app.py |
